...
class Pendaftaran extends CI_Controller
{
	...
	function autocomplate_ruangan_dirawat(){
        autocomplate_json('tbl_tempat_tidur', 'kode_tempat_tidur');
    }
    ...
    public function create_action() 
    {
		...
		'asal_rujukan' => $this->input->post('asal_rujukan',TRUE),
            ...
            redirect(site_url('pendaftaran/detail/'.$this->input->post('no_rawat',TRUE)));
        }
    }
    ...
    function cetak_riwayat_labor(){
        ...
        $pdf->Image(base_url().'assets/foto_profil/'.  getInfoRS('logo'),20,5,25);
        //$pdf->Image($file, $x, $y, $w, $h)
        
        // mencetak string 
        $pdf->Cell(250, 7, getInfoRS('nama_rumah_sakit'), 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(250, 6, getInfoRS('alamat'), 0, 1, 'C');
        $pdf->Cell(250, 6, 'No Telpon : +62896232323  Email : rspurwokerto@gmail.com', 0, 1, 'C');
        $pdf->Line(10, 33, 260-0, 33);
        $pdf->Cell(8, 8, '',0,1);
		$pdf->SetFont('Arial', 'B', 11);
        $pdf->Cell(250, 10, 'HASIL PEMERIKSAAN LABORATORIUM', 0, 1, 'C');
        ...
        // data pasien
         ...
         $pdf->Cell(60, 6, ': '.$pasien['no_rekamedis'], 0, 0, 'l');
         ...
         $pdf->Cell(60, 6, ': '.$pasien['nama_pasien'], 0, 0, 'l');
         ...
         foreach ($pemeriksaan as $p){
            ...
            foreach ($sub_periksa as $s){
                ...
                $pdf->Cell(50, 7, $s->hasil, 1, 0, 'C');
                $pdf->Cell(50, 7, $s->satuan, 1, 0, 'C');
    ...
    function catak_rekamedis(){
        ...
        $sql_daftar = "SELECT pd.no_rekamedis, pd.no_rawat, pd.tanggal_daftar, ps.nama_pasien, ps.nama_ibu, a.agama, ps.tanggal_lahir, sm.status_menikah, pe.nama_pekerjaan, ps.alamat, pd.nama_penanggung_jawab, pd.alamat_penanggung_jawab, jb.jenis_bayar, pd.cara_masuk, pol.nama_poliklinik FROM 
                        tbl_pendaftaran as pd,tbl_pasien as ps, tbl_agama as a, tbl_pekerjaan as pe, tbl_status_menikah as sm, tbl_jenis_bayar as jb, tbl_poliklinik as pol 
                        WHERE pd.no_rekamedis=ps.no_rekamedis and ps.id_agama=a.id_agama and ps.id_pekerjaan=pe.id_pekerjaan and ps.status_menikah=sm.id_status_menikah and pd.id_jenis_bayar=jb.id_jenis_bayar and pd.id_poli=pol.id_poliklinik and pd.no_rawat='$no_rawat'"; 
        $sql_tindakan = "SELECT tt.*,tr.hasil_periksa,tr.tanggal,tr.perkembangan 
                        FROM tbl_riwayat_tindakan as tr,tbl_tindakan as tt
                        WHERE tr.kode_tindakan=tt.kode_tindakan and tr.no_rawat='$no_rawat'";
        $sql_obat     = "SELECT ta.kode_barang,ta.nama_barang,ta.harga,tp.tanggal,tp.jumlah
		...
		$pdf->SetTitle($pendaftaran['no_rawat']);
        // membuat halaman baru
        $pdf->AddPage();
		
		$pdf->SetFont('Arial', 'B', 14);
        
        $pdf->Image(base_url().'assets/foto_profil/'.  getInfoRS('logo'),10,5,25);
        //$pdf->Image($file, $x, $y, $w, $h)
        
        // Header Cetak Rekamedis 
        
		$pdf->Cell(200, 7, strtoupper(getInfoRS('nama_rumah_sakit')), 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(200, 6, getInfoRS('alamat'), 0, 1, 'C');
        $pdf->Cell(200, 6, 'No Telpon : +62896232323  Email : simrs@gmail.com', 0, 1, 'C');
        $pdf->Line(10, 33, 200-0, 33);
        $pdf->Cell(8, 8, '',0,1);
		$pdf->SetFont('Arial', 'B', 11);
        ...
        $pdf->Line(10, 45, 200-0, 45);
        $pdf->SetY(29);
		$pdf->Cell(190, 20, 'CETAK NO REKAMEDIS', 0, 1, 'C');
        
		// No Rawat, No Rekamedis, Cara Masuk, Jenis Pembayaran
		
		$pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(40, 15, 'Tanggal : ', 0, 0, 'L');
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell(60, 15, $pendaftaran['tanggal_daftar'], 0, 0, 'L');
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(40, 15, 'Tujuan Poliklinik : ', 0, 0, 'L');
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell(60, 15, $pendaftaran['nama_poliklinik'], 0, 1, 'L');
		$pdf->SetFont('Arial', '', 10);
        $pdf->Cell(40, 10, 'No Rawat', 1, 0, 'C');
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(60, 10, $pendaftaran['no_rawat'], 1, 0, 'C');
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell(30, 10, 'No Rekamedis', 1, 0, 'C');
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(60, 10, $pendaftaran['no_rekamedis'], 1, 1, 'C');
		$pdf->SetFont('Arial', '', 10);
        $pdf->Cell(40, 10, 'Cara Masuk', 1, 0, 'C');
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(60, 10, $pendaftaran['cara_masuk'], 1, 0, 'C');
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell(30, 10, 'Pembayaran', 1, 0, 'C');
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(60, 10, $pendaftaran['jenis_bayar'], 1, 1, 'C');
		$pdf->Cell(60, 3, '', 0, 1, 'C');
		
        /*
		$pdf->Cell(95, 7, 'Nama Pasien       : '.$pendaftaran['nama_pasien'], 1, 0, 'l');
		$pdf->Cell(95, 7, 'Nama Ibu       : '.$pendaftaran['nama_ibu'], 1, 1, 'l');
        
        $pdf->Cell(130, 7, 'No Identitas         : '.$pendaftaran['no_rawat'], 1, 0, 'l');
        ...
        */
		$pdf->SetFont('Arial', '', 11);
		$pdf->Cell(190, 7, 'Nama Pasien               : '.$pendaftaran['nama_pasien'], 0, 1, 'l');
        $pdf->Cell(190, 7, 'Nama Ibu                     : '.$pendaftaran['nama_ibu'], 0, 1, 'l');
        $pdf->Cell(190, 7, 'Agama                         : '.$pendaftaran['agama'], 0, 1, 'l');
        $pdf->Cell(190, 7, 'Tanggal Lahir              : '.$pendaftaran['tanggal_lahir'], 0, 1, 'l');
        $pdf->Cell(190, 7, 'Status                          : '.strtoupper($pendaftaran['status_menikah']), 0, 1, 'l');
        $pdf->Cell(190, 7, 'Pekerjaan                    : '.$pendaftaran['nama_pekerjaan'], 0, 1, 'l');
        $pdf->Cell(90, 7, 'Alamat                         : '.$pendaftaran['alamat'], 0, 1, 'l');
		$pdf->Cell(8, 8, '',0,1);
		$pdf->Cell(40, 14, 'Penanggung Jawab', 1,0, 'C');
        $pdf->Cell(150, 7, 'Nama       : '.$pendaftaran['nama_penanggung_jawab'], 1, 1, 'l');
        $pdf->Cell(40,7,'',0,0);
        $pdf->Cell(150, 7, 'Alamat     : '.$pendaftaran['alamat_penanggung_jawab'], 1, 1, 'l');
		$pdf->Cell(8, 3, '',0,1);
        
		$pdf->Cell(190,7,'*) Lingkari yang sesuai',0,1);
        
		$pdf->SetY(-125);
		$pdf->SetFont('Arial', 'B', 11);
		$pdf->Cell(64, 7, 'Riwayat Pemeriksaan',0,0);
		$pdf->Cell(64, 7, 'Riwayat Tindakan',0,0);
		$pdf->Cell(64, 7, 'Resep Obat',0,1);
		
		$pdf->SetFont('Arial', '', 10);
		$pdf->Cell(64, 7, '- Riwayat Pemeriksaan',0,1);
		$pdf->Cell(64, 7, '- Riwayat Tindakan',0,0);
		$pdf->Cell(64, 7, '- Resep Obat',0,1);
